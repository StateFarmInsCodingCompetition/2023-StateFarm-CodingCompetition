...
class TestSetOne:
    ...
    def test_get_num_disasters_for_state(self, controller):
        """Test 3"""
        assert controller.get_num_disasters_for_state("Arizona") == 2
        assert controller.get_num_disasters_for_state("Georgia") == 5
        assert controller.get_num_disasters_for_state("Illinois") == 2
        assert controller.get_num_disasters_for_state("Texas") == 9
        assert controller.get_num_disasters_for_state("District of Columbia") == 2
...
class TestSetTwo:
    def test_get_total_claim_cost_for_disaster(self, controller):
        ...
        assert controller.get_total_claim_cost_for_disaster(56) == 1287476.19
        assert controller.get_total_claim_cost_for_disaster(101) == None
        assert controller.get_total_claim_cost_for_disaster(78) == 614822.68

    def test_get_average_claim_cost_for_claim_handler(self, controller):
        """Test 5"""
        assert controller.get_average_claim_cost_for_claim_handler(2) == 87330.89
        assert (
            round(controller.get_average_claim_cost_for_claim_handler(42), 2)
            == 122195.90
        )
        ...
        assert (
            round(controller.get_average_claim_cost_for_claim_handler(151), 2)
            == 242134.96
        )
    ...
    def test_get_most_spoken_agent_language_by_state(self, controller):
        """Test 7"""
        assert (
            controller.get_most_spoken_agent_language_by_state("New Hampshire")
            == "Arabic"
        )
        assert controller.get_most_spoken_agent_language_by_state("Wisconsin") == ""
        assert (
            controller.get_most_spoken_agent_language_by_state("Florida") == "Spanish"
        )

    def test_get_num_of_open_claims_for_agent_and_severity(self, controller):
        """Test 8"""
        assert controller.get_num_of_open_claims_for_agent_and_severity(0, 0) == -1
        assert controller.get_num_of_open_claims_for_agent_and_severity(25, 11) == -1
        assert controller.get_num_of_open_claims_for_agent_and_severity(65, 3) == None
        assert controller.get_num_of_open_claims_for_agent_and_severity(24, 1) == 16
        assert controller.get_num_of_open_claims_for_agent_and_severity(87, 6) == 3
        assert controller.get_num_of_open_claims_for_agent_and_severity(85, 6) == 2
...
class TestSetThree:
    ...
    def test_build_map_of_agents_to_total_claim_cost(self, controller):
        ...
        expected_agent_ids_without_cost = [
            2,
            6,
            9,
            12,
        ...
        for i in range(3):
            random_agent_id = expected_agent_ids_without_cost[
                random.randint(0, num_agent_ids_without_cost - 1)
            ]
...
class TestSetFour:
    def test_get_top_three_months_with_highest_num_of_claims_desc(self, controller):
        """Test 12"""
        top_three_months = (
            controller.get_top_three_months_with_highest_num_of_claims_desc()
        )
